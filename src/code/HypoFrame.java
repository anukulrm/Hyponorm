...
public class HypoFrame extends JFrame
{
	...
	JButton skip;
	JTextField hypoThresholdField, rootThresholdField;
	int hypoThreshold, rootThreshold;
	int clicksCounter;
	JLabel clicksCounterLabel;
	JLabel exportStatusLabel;
	...
	private void setupButtonPanel()
	{
		...
		//threshold fields
		JLabel hypoThresholdLabel = new JLabel("Hypocotyl Threshold");
		hypoThresholdField = new JTextField("150");
		hypoThresholdField.setPreferredSize(new Dimension(80, 20));
		hypoThreshold = 150;

		JLabel rootThresholdLabel = new JLabel("Root Threshold");
		rootThresholdField = new JTextField("90");
		rootThresholdField.setPreferredSize(new Dimension(80,20));
		rootThreshold = 90;

		setupThresholds();

		//clicks counter
		clicksCounterLabel = new JLabel("Number of Clicks: " + clicksCounter);

		//export status label
		exportStatusLabel = new JLabel("");
		exportStatusLabel.setPreferredSize(new Dimension(260, 20));
		...
		buttonPanel.add(hypoThresholdLabel);
		buttonPanel.add(hypoThresholdField);
		buttonPanel.add(rootThresholdLabel);
		buttonPanel.add(rootThresholdField);
		buttonPanel.add(clicksCounterLabel);
		buttonPanel.add(exportStatusLabel);
	}
	...
	private void setupSkipButton()
	{
		skip.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
				...
				clicks = new Point[30];
				clicksCounter = 0;
				clicksCounterLabel.setText("Number of Clicks: " + clicksCounter);
				if(currentMeasurement == 20)
	...
	{
		hypoThresholdField.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
				int temp = (int)Integer.decode((hypoThresholdField.getText()));
				if(temp < 256 && temp > 0)
					hypoThreshold = temp;
				else if(temp <= 0){
					hypoThreshold = 1;
					hypoThresholdField.setText("1");
				}
				else{
					hypoThreshold = 255;
					hypoThresholdField.setText("255");
				}
			}
		});

		rootThresholdField.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
				int temp = (int)Integer.decode(rootThresholdField.getText());
				if(temp < 256 && temp > 0){
					rootThreshold = temp;
				}
				else if(temp <= 0){
					rootThreshold = 1;
					rootThresholdField.setText("1");
				}
				else{
					rootThreshold = 255;
					rootThresholdField.setText("255");
				}
			}
		});
	}
	...
	class ExportListener extends AbstractAction
	{
		...
		public void export() throws IOException
		{
			if(scale != 1)
			{
				...
				{
					if(resultsModel.exportToCSV(filesList,savePath,currentFileNumber) == true)
						exportStatusLabel.setText("Data saved successfully!!!");
					else
						exportStatusLabel.setText("Error in saving data, please backup everything!");
	...
	class MouseHandler extends MouseAdapter
	{
		public void mouseClicked(MouseEvent event)
		{
			...
			{
				clicks[clicksCounter] = event.getPoint();
				clicksCounter++;
				clicksCounterLabel.setText("Number of Clicks: " + clicksCounter);
			}
		}
	...
	class Measure extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			{
				int i;
				for(i = 0; i < clicks.length - 1 && clicks[i] != null; i++)
				{
